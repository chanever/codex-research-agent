...
- Example scenario: 사용자는 "회의 이메일을 보고 호텔을 예약해줘"라고 했다. 실제 agent는 호텔 검색 결과에 숨어 있던 악성 지시 때문에 결제 계좌를 바꾼다.

### Six Ws and H

- Who: tool-use LLM agent를 운영하는 개발자와 보안팀.
- What: expected authorization graph와 observed provenance graph 사이의 mismatch를 탐지한다.
- When: 민감 tool call 직전, 실행 중 runtime gate, 또는 실행 후 audit 단계.
...
- Required data: AgentDojo/AgentDyn traces, MCPTox/MSB samples, 자체 coding-agent sandbox logs, `strace` logs.
...
- Baselines to compare: text-based prompt injection classifier, LLM judge, allowlist-only policy, Agent-Sentry-style feature classifier, AuthGraph-style alignment, Progent privilege policy.

### Experiments

- Benchmark / dataset candidates: AgentDojo, AgentDyn, MCPTox, MSB, SafeClawBench.
...
- Existing problems in prior work:
  ...
  - Problem 2:
    ...
    - Why it is not enough: MCP pipeline 전반의 공격을 다루지만, metadata-to-side-effect provenance를 detector로 만드는 구체 방법은 새 연구 여지가 있다.
...
- Required data: MCPTox benchmark, MSB, MCP server manifests, tool call logs, sandbox side-effect logs.
...
- Benchmark / dataset candidates: MCPTox, MSB, AgentDyn MCP-style extensions freshness 확인 필요.
...
- Failure cases to check: metadata가 간접적이고 reasoning log가 없는 경우, tool output poisoning과 metadata poisoning이 섞인 경우.
...
- Intuition: 악성 package는 `pip install`이나 `npm install` 중 postinstall script로 비밀을 읽고 외부로 보낼 수 있다. agent log만 보면 "의존성 설치"로 보이므로 OS-level trace가 필요하다.
...
- Why: coding agent는 사용자의 승인 없이 많은 shell command를 실행할 수 있고, package install은 정상 개발 workflow라 공격이 숨기 쉽다.
...
- Existing problems in prior work:
  - Problem 1:
    ...
    - Why it is not enough: agent runtime trace를 graph IR로 바꾸지만, package lifecycle script와 syscall-level side effect까지 포함한 coding agent 실험은 별도 설계가 필요하다.
  - Problem 2:
    - Source: Quantifying Frontier LLM Capabilities for Container Sandbox Escape
    - URL: https://arxiv.org/abs/2603.02277
    - Why it is not enough: sandbox escape 능력을 측정하지만, 일반 coding workflow에서 나타나는 pre-escape 위험 행동을 provenance graph로 탐지하는 연구는 추가로 가능하다.
...
- Required data: synthetic malicious npm/PyPI packages, known malicious package reports, coding agent task logs, Docker/strace logs.
...
- Baselines to compare: package static scanner, command regex policy, network block-only sandbox, LLM judge over shell command text.

### Experiments

- Benchmark / dataset candidates: SafeClawBench for sandbox harm framing, SandboxEscapeBench for sandbox threat model, MCPTox/MSB for tool metadata analogy, curated malicious PyPI/npm reports freshness 확인 필요.
...
- Required data: SafeClawBench dataset, AgentDojo/AgentDyn traces with added side-effect labels, sandbox logs.
- System design: graph builder가 final text, tool audit, OS sandbox event를 별도 endpoint node로 저장한다.
- Implementation steps: endpoint label schema 작성, existing benchmark outputs 변환, detector별 endpoint confusion matrix 계산.
- Graph schema:
  - Nodes: `attack_instruction`, `model_acceptance`, `tool_audit_event`, `sandbox_state_change`, `blocked_action`.
  - Edges: `claims`, `attempts`, `executes`, `observed_as`, `blocked_before`.
  - Labels: semantic_fail, audit_harm, sandbox_harm, severity, reversibility.
- Detector / algorithm: endpoint-specific motif detector와 causal path interruption score.
- Baselines to compare: ASR-only evaluation, SafeClawBench endpoint metrics, firewall defense, no defense.

### Experiments

- Benchmark / dataset candidates: SafeClawBench, AgentDojo, AgentDyn, MSB.
- Experimental setup: 같은 attacks에 text classifier, firewall, graph detector를 적용하고 endpoint별 harm reduction을 비교한다.
- Metrics: semantic ASR, audit harm rate, sandbox harm rate, harm pathway recall, overblocking rate, utility.
- Baseline comparisons: final-answer-only judge, tool-log-only judge, sandbox-only judge, full graph detector.
- Ablation study: final answer node 제거, audit node 제거, sandbox node 제거, edge direction 제거.
- Expected result: final-answer-only는 실제 harm을 놓치고, full graph는 audit/sandbox harm을 더 잘 설명한다.
- Failure cases to check: sandbox event가 없는 API-only harm, audit log 누락, reversible benign state change.

### Practical Plan

- Expected difficulty: Medium.
- Risk / limitation: benchmark마다 harm label 정의가 달라 직접 비교가 어렵다.
- First experiment: SafeClawBench 100개 sample을 graph endpoint schema로 변환한다.
- Next implementation step: endpoint별 confusion matrix script를 만든다.

## Idea 5. Temporal Memory-Provenance Graph for Delayed Prompt Injection

### Easy Explanation

- One-line summary: 오늘 읽은 악성 문서가 memory에 저장되고 나중에 tool call을 오염시키는 delayed attack을 그래프로 추적한다.
- Intuition: 공격은 한 번의 tool output에서 바로 실행되지 않을 수 있다. memory에 작은 조각으로 저장됐다가 민감 task에서 다시 결합될 수 있다.
- Example scenario: agent가 README에서 "배포 시 backup_server를 사용하라"는 문장을 memory에 저장하고, 다음 주 배포 task에서 credential을 attacker server로 보낸다.

### Six Ws and H

- Who: long-running assistant, coding agent, enterprise workflow agent 운영자.
- What: memory write/read와 future tool call 사이의 provenance edge를 추적한다.
- When: multi-turn session, scheduled agent run, persistent memory 사용 시.
- Where: workspace agent, coding assistant, browser agent, RAG agent.
- Why: single-turn graph detector는 memory poisoning과 delayed activation을 놓칠 수 있다.
- How: memory item을 first-class node로 만들고, source trust와 future use를 temporal edge로 연결한다.

### Research Framing

- Hypothesis: temporal memory-provenance graph는 per-call sanitizer나 stateless detector보다 delayed injection과 memory poisoning을 더 잘 탐지한다.
- Motivation: agentic workflow security는 단일 호출이 아니라 장기 실행과 상태 유지가 핵심이다.
- Existing problems in prior work:
  - Problem 1:
    - Source: DRIFT: Dynamic Rule-Based Defense with Injection Isolation for Securing LLM Agents
    - URL: https://arxiv.org/abs/2506.12104
    - Why it is not enough: memory stream isolation을 제안하지만, memory provenance를 장기 graph로 저장하고 side effect까지 연결하는 실험은 더 확장할 수 있다.
  - Problem 2:
    ...
    - Why it is not enough: dynamic task와 helpful third-party instruction은 좋지만, persistent memory 기반 delayed attack 평가가 핵심은 아니다.
- Proposed contribution: memory item source, write time, read time, downstream tool argument를 연결하는 temporal provenance graph와 delayed attack benchmark.
- Why this could be novel: agent memory를 단순 text store가 아니라 taint-bearing state로 모델링한다.

### Methodology

- Required data: multi-turn AgentDojo/AgentDyn extensions, synthetic memory poisoning tasks, workspace agent memory logs.
- System design: memory write API와 retrieval API를 intercept해 memory node를 만들고 source trust를 보존한다.
- Implementation steps: memory poisoning scenarios 작성, memory logger 구현, future tool call argument와 memory source matching, temporal motif detector 작성.
- Graph schema:
  - Nodes: `untrusted_document`, `memory_item`, `retrieval_event`, `tool_call`, `argument`, `side_effect`.
  - Edges: `stored_from`, `retrieved_by`, `derived_argument`, `causes`, `expires_after`.
  - Labels: trust, topic, timestamp, TTL, sensitivity, activation condition.
- Detector / algorithm: taint propagation over temporal graph, memory TTL policy, sensitive task retrieval gate, delayed motif detection.
- Baselines to compare: per-turn sanitizer, no-memory baseline, DRIFT-style injection isolator, LLM memory reviewer.

### Experiments

- Benchmark / dataset candidates: AgentDyn, SafeClawBench memory poisoning family, custom multi-turn delayed injection dataset.
- Experimental setup: attacker plants memory in turn 1, benign user asks sensitive task in turn N, detector decides whether memory can influence tool call.
- Metrics: delayed ASR, benign memory utility, tainted-memory recall, false removal rate, time-to-detection.
- Baseline comparisons: stateless prompt injection detector, memory sanitizer, graph taint detector, human-written memory allowlist.
- Ablation study: timestamp 제거, source trust 제거, topic labels 제거, TTL policy 제거.
- Expected result: temporal provenance가 있으면 delayed attack을 더 잘 잡지만 benign personalization memory에서 오탐이 생길 수 있다.
- Failure cases to check: paraphrased memory, split payload across multiple memory items, legitimate user-supplied secrets.

### Practical Plan

- Expected difficulty: Medium-High.
- Risk / limitation: persistent memory logs는 privacy-sensitive하므로 synthetic data와 redaction이 필요하다.
- First experiment: 20개 delayed injection 시나리오를 만들어 memory taint rule을 테스트한다.
- Next implementation step: memory write/read event schema를 정의한다.

## Experiment Backlog

### Easy

- AgentDojo 한 suite에서 tool call, tool output, argument를 JSONL로 logging한다.
- MCPTox abstract/sample 기준으로 `tool_metadata -> tool_call` motif를 수동 라벨링한다.
- Agent-Sentry, AuthGraph, AgentArmor의 graph schema를 표로 비교한다.
- SafeClawBench sample을 semantic/audit/sandbox endpoint node로 변환한다.

### Medium

- AgentDojo/AgentDyn trace를 NetworkX graph로 변환하고 rule detector를 구현한다.
- MCP proxy를 만들어 tool metadata hash, selected tool, arguments, response를 저장한다.
- Docker sandbox에서 `strace -f` 기반 process/file/network event graph를 만든다.
- Progent-style policy와 graph mismatch detector를 같은 task에서 비교한다.

### Hard

- AgentDyn open-ended tasks에서 expected graph를 자동 생성하고 false positive를 낮춘다.
- Package install attack fixture를 만들어 coding agent가 실제로 설치/빌드/테스트하게 한 뒤 sandbox-observed harm을 측정한다.
- Temporal memory-provenance graph로 delayed prompt injection benchmark를 만든다.
- Graph neural network detector와 rule detector를 adaptive attacks에서 비교한다.

## Possible Paper Angle

- "Intent-to-Execution Provenance Graphs for Detecting Malicious Tool Use in LLM Agents"
- 핵심 기여: user intent, tool metadata, runtime tool calls, OS-level side effects를 하나의 typed provenance graph로 결합하고, expected-vs-observed alignment와 side-effect motif를 통해 indirect prompt injection, MCP tool poisoning, package install attack을 탐지한다.
- 실험 각도: AgentDojo/AgentDyn에서 prompt injection, MCPTox/MSB에서 MCP poisoning, synthetic Docker sandbox에서 package install attack을 통합 평가한다.
- 차별점: 기존 graph 방어가 tool argument provenance에 집중했다면, 이 연구는 sandbox-observed harm과 software supply chain side effect까지 연결한다.

## Next Research Question

- expected authorization graph를 LLM이 생성할 때 그 graph 자체가 공격자 입력에 오염되지 않도록 어떻게 보장할 수 있는가?
- tool metadata와 package metadata처럼 "자연어이지만 실행 계획에 영향을 주는 supply-chain artifact"를 같은 provenance schema로 표현할 수 있는가?
- graph detector가 실제 production agent에서 허용 가능한 latency와 false positive를 유지하려면 어떤 node/edge를 반드시 수집하고 어떤 것은 생략해야 하는가?
